(function(a,c) {
	var b=function(e,d) {
		...
		this.$element.on("click",".tree-item",
			a.proxy(function(f) {this.selectItem(f.currentTarget)},
			this));
		this.$element.on("click",".tree-folder-header",
			a.proxy(function(f) {this.selectFolder(f.currentTarget)},
			this));
		this.render()};
	...
	b.prototype= {
		...
		populate: function(f) {
			...
			this.options.dataSource.data(f.data(), function(g) {
				...
				a.each(g.data, function(h, j) {
					var i;
					if(j.type==="folder") {
						i=e.$element.find(".tree-folder:eq(0)").clone().attr({'layer':j.layer, 'folder':j.folder}).show();
						...
						if(j.type==="item") {
							i=e.$element.find(".tree-item:eq(0)").clone().attr({'layer':j.layer, 'folder':j.folder, 'param':j.param}).show();
		...
		selectItem: function(e) {
			...
			var d=a(e);
			if(d.find("b").attr("editing")=="true") { return; }
			...
			if(d.hasClass("tree-selected")) {
				...
				this.$element.trigger("deselected", {selected:f, target:d});
			}
			else {
				d.addClass("tree-selected");
				d.find("i").removeClass(this.options["unselected-icon"]).addClass(this.options["selected-icon"]);
				if(this.options.multiSelect) { f.push(d.data()); }
				this.$element.trigger("selected", {selected:f, target:d});
			}
		},
		...
		selectFolder: function(e) {
			...
			if(d.find("."+this.options["close-icon"]).length) {
				...
				this.$element.trigger(behavior, d.parent());
			}
			else {
